...
* Security Enhanced Linux
  * security about permission settings
    * Discretionary Access Control (DAC)
      * e.g. root has the highest permission and users can change the permission
      * risk: if users carelessly set permission to 777
    * Mandatory Access Control (MAC)
      * SELinux use it
        * when you are a root, you are not always the root in all programs. Your permission is determined by the program.
      * with many policies and rules to have permission
    ![DAC vs MAC](dac_mac.jpg)
* Process of SELinux
  * the subject that SELinux mainly manages is **program**
  * the object is the target file system to be accessed by the subject
  * SELinux has different policies and rules to limit the access of the objects
  * after the policy, the security context of the program should the same as the one of the object. Like `rwx`. If wrong, the error of `wrong permissions` jumps out.
  ![selinux](selinux.gif)
<br />
...
### lsof 
* list all running files or devices


```bash
  lsof [-aUu] [+d]

  # -a: connect multiple conditions: e.g. -u + -U
  lsof -u root -a -U

  # -d: find the open files under the directory
  lsof +d /dev
...
### Security Context 
* after activating SELinux, users can see the Security Context
...
```
* Table of columns
  |Columns|Meaning|Note|
  |:---:|:---|:---|
  |Identify|unconfined_u: files generated by programs which are not limited permissions by SELinux; system_u: files generated by users|default bash is not limited by SELinux. So, files created by users in bash are all unconfined_u|
  |role|identify if the data is programs, files or users. object_r: directories, files; system_r: programs, general users||
  |Type|**Most important**. Define whether an object can be accessed by files. type: on the files; domain: on the programs|programs can access to files if they have type and domain together|
...
```bash
  ...
  # the third position is the security context
```

* the identify and role map under the policy of target
  |identify|role|Meaning|
  |:---:|:---:|:---|
  |unconfined_u|unconfined_r|not limited programs|
  |system_u|system_r|system accounts|