...
- [Commands](#commands)
  ...
  - [linking files](#linking-files)
    ...
    - [the link numbers of directories](#the-link-numbers-of-directories)
...
## Disk Space
### df
* display the usage of disks => superblock
...
```s
  ...
  # display the used space of the current directory will all files in a easy way to see the usage
  du -ah

  # all files under root
  du -sm /*

  # not include the sum of subdirectories
  du -S
...
## linking files
* there are tow ways of linking files
  1. Symbolic Links: like Windows shortcut
  2. Hard Links: create new file names by inode links, instead of new files

<br />

### Symbolic Links
* shortcuts; only read the inode of target files 
* a new file with different inode number points to a file name
  > increase the number of inode and blocks 
* if source files are deleted, the symbolic linking files can not open
  

```s
  ln -s <files_to_be_linked> <new_file>

  # with different inode numbers
  ## the sixth column of the new file means the size of the linking file based on the number of letters of the file name 
  ll -i <files_to_be_linked> <new_file>
...
* add a new file name under the directory to point to a specific inode number 
* safe to change inode and block data. Even if you delete one file name, there are still other file names pointing to the same inode number
* will not increase the number of inode and blocks
  > hard links will increase the number of blocks if the space of existing blocks is not enough
* pay attention
  1. can not cross across filesystem (every filesystem with their own superblock, inode, blocks)
  2. [can not link to directories](https://linux.vbird.org/linux_basic/centos7/0230filesystem.php#filesys_1)
     > may lead to many parent directories pointing to the same directory
...
```s
  ...
  # create hard links
  ln <file_to_be_linked> <path>
  
  # check inode
  ## the second column means how many file names link to the inode number 
  ll -i <file_to_be_linked> <hard_link_files>

```

<br />

### the link numbers of directories
* when creating a new directory, three directories are generated by default:
  * /tmp/testing     => /tmp/testing
  * /tmp/testing/.   => /tmp/testing
  * /tmp/testing/..  => /tmp
* the change of link numbers when creating a new directory under the existing directory:
  * /tmp => add **one** link 
  * /tmp/testing => with **two** links